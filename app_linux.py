...
import numpy as np
import collections
import os # Linux 환경 변수 설정을 위해 추가

# ------------------ Linux 환경 설정 ------------------
# Matplotlib 백엔드 명시 (Tkinter와 충돌 방지 및 안정성 확보)
...
# 카메라: Linux (V4L2) 백엔드 명시
# Arch Linux에서는 0번 카메라가 기본적으로 V4L2를 사용할 가능성이 높지만, 명시적으로 지정
cap = cv2.VideoCapture(0, cv2.CAP_V4L2)
cap.set(cv2.CAP_PROP_FRAME_WIDTH, CAM_W)
cap.set(cv2.CAP_PROP_FRAME_HEIGHT, CAM_H)

# ------------------ 사운드 유틸 (beepy 수정) ------------------
# beepy의 사운드 객체를 미리 로드 (Linux에서 더 안정적)
try:
    sound_inc = beepy.beep(sound=1, play=False)
    sound_dec = beepy.beep(sound=2, play=False)
    sound_reset = beepy.beep(sound=3, play=False)
    # trade 사운드는 연속음을 위해 리스트로 저장
    sound_trade = [beepy.beep(sound=i, play=False) for i in [4, 5, 6]] 
except Exception as e:
    print(f"사운드 초기화 오류: {e}. 사운드 기능이 작동하지 않을 수 있습니다.")
    # 오류 발생 시 더미 함수로 대체
    def dummy_fn(*args, **kwargs): pass
    sound_inc = sound_dec = sound_reset = dummy_fn
    sound_trade = [dummy_fn, dummy_fn, dummy_fn]

def _async(fn):
    threading.Thread(target=fn, daemon=True).start()

def snd_inc():
    _async(lambda: sound_inc.play() if hasattr(sound_inc, 'play') else None)

def snd_dec():
    _async(lambda: sound_dec.play() if hasattr(sound_dec, 'play') else None)

def snd_reset():
    _async(lambda: sound_reset.play() if hasattr(sound_reset, 'play') else None)

def snd_trade():
    def play_sequence():
        for s in sound_trade:
            if hasattr(s, 'play'):
                s.play()
                time.sleep(0.1) # 연속음 간격
    _async(play_sequence)
...
def main_loop():
    ...
    ret, frame = cap.read()
    if not ret:
        print("경고: 카메라 프레임을 읽을 수 없습니다. (카메라 연결 확인)")
        root.after(int(1000 * TICK_DT), main_loop) # 계속 루프 재호출 시도
        return
            ...
            if detect_fist(hand_landmarks):
                ...
                if gesture == "INC":
                    ...
                    if current_time - last_inc_sound >= SND_COOLDOWN_INC:
                        snd_inc(); last_inc_sound = current_time # 수정된 snd_inc 호출
                elif gesture == "DEC":
                    order_amount = int(order_amount) - STEP_NORMAL
                    order_entry.delete(0, 'end'); order_entry.insert(0, str(int(order_amount)))
                    if current_time - last_dec_sound >= SND_COOLDOWN_DEC:
                        snd_dec(); last_dec_sound = current_time # 수정된 snd_dec 호출
